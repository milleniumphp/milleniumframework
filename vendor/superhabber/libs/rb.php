...
;
    ...
    class OODBBean implements\IteratorAggregate, \ArrayAccess, \Countable, Jsonable {
        ...
        public static function setErrorHandlingFUSE($mode, $func = NULL) {
            if (
                    $mode !== self::C_ERR_IGNORE && $mode !== self::C_ERR_LOG && $mode !== self::C_ERR_NOTICE && $mode !== self::C_ERR_WARN && $mode !== self::C_ERR_EXCEPTION && $mode !== self::C_ERR_FUNC && $mode !== self::C_ERR_FATAL
            )
        ...
        public function beau($property) {
            ...
            if (
                    ( strpos($property, 'own') === 0 && ctype_upper(substr($property, 3, 1)) ) || ( strpos($property, 'xown') === 0 && ctype_upper(substr($property, 4, 1)) ) || ( strpos($property, 'shared') === 0 && ctype_upper(substr($property, 6, 1)) )
            ) {
        ...
        public function equals(OODBBean $bean) {
            return (bool) (
                    ( (string) $this->properties['id'] === (string) $bean->properties['id'] ) && ( (string) $this->__info['type'] === (string) $bean->__info['type'] )
                    );
        }
            ...
            foreach ($bean as $prop => $value) {
                if (
                    ...
                    throw new RedException("Invalid Bean value: property $prop");
                        ...
                        strlen($prop) < 1 || preg_match($pattern, $prop)
                ) {
            ...
            if (is_array($typeOrBeanArray)) {
                ...
                if (!isset($typeOrBeanArray['_type'])) {
                    ...
                    foreach ($typeOrBeanArray as $beanArray) {
                        if (
                                !( is_array($beanArray) && isset($beanArray['_type']) )) {
